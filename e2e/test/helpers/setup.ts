...
import { createManyArticles as createManyArticlesViaAPI } from './api'
...
export async function createManyArticles(
  ...
  if (API_MODE && request && token) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await page.fill(
        'input[name="title"]',
        `Test Article ${uniqueId} Number ${i}`,
      )
      await page.fill(
        'input[name="description"]',
        `Description for test article ${i}`,
      )
      await page.fill(
        'textarea[name="body"]',
        `Body content for test article ${i}. Created with ID ${uniqueId}.`,
      )
