...
test.beforeEach(({}, testInfo) => {
  testInfo.skip(
    !API_MODE,
    'API-only: all tests use page.route() + localStorage',
  )
})
...
test.describe('User Fetch Errors on App Initialization - 5XX (should enter unavailable mode)', () => {
  test('should handle 500 Internal Server Error on /api/user', async ({
    page,
  }) => {
    await page.route(`${API_BASE}/user`, (route) => {
      route.fulfill({
  ...
  test('should allow browsing app in unavailable mode and persist token across reloads', async ({
    page,
  }) => {
...
test.describe('User Fetch Errors - Protected Routes', () => {
  ...
  test('should handle 500 on loading /settings gracefully', async ({
    page,
  }) => {
    await page.route(`${API_BASE}/user`, (route) => {
      route.fulfill({
