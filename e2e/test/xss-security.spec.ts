...
test.beforeEach(({}, testInfo) => {
  testInfo.skip(
    !API_MODE,
    'API-only: all tests use direct API calls + localStorage injection',
  )
})
...
const XSS_IMAGE_PAYLOADS = [
  ...
  },
  {
    ...
    payload:
      'https://example.com/img.jpg"onerror="javascript:alert(document.cookie)',
  },
  {
...
test.describe('@security XSS Security - Image URL Injection (Direct API)', () => {
  for (const { name, payload } of XSS_IMAGE_PAYLOADS) {
    test(`should prevent XSS via ${name}`, async ({ page, request }) => {
      ...
      const hasOnload = await imgElement.evaluate((el) =>
  ...
  test('should safely render malicious payload in navbar image', async ({
    page,
    request,
  }) => {
    const wasXssTriggered = setupXssDetector(page)
    ...
    const maliciousImage =
      'https://x.com/img.jpg"onerror="alert(document.cookie)'
  ...
  test('should safely render malicious payload in article comments', async ({
    page,
    request,
  }) => {
    const wasXssTriggered = setupXssDetector(page)
...
test.describe('@security XSS Security - Article Description in Feed (Direct API)', () => {
  ...
  for (const { name, payload } of XSS_DESCRIPTION_PAYLOADS) {
    test(`should sanitize ${name} in article description`, async ({
      page,
      request,
    }) => {
      const wasXssTriggered = setupXssDetector(page)
...
test.describe('@security XSS Security - Article Body Markdown (Direct API)', () => {
  for (const { name, payload } of XSS_MARKDOWN_PAYLOADS) {
    test(`should sanitize ${name} in article body`, async ({
      page,
      request,
    }) => {
      const wasXssTriggered = setupXssDetector(page)
      ...
      for (const handler of dangerousHandlers) {
        const elementsWithHandler = await articleBody
          .locator(`[${handler}]`)
          .count()
